...
function Header({ title = "Title",activePage }) {
  ...
  return (
    <div
      id="header"
      className={  `fixed top-0  ${isOpen ? 'justify-end ' : 'justify-between right-0'} z-10  bg-[#fcfcfc] h-16 w-screen min-[900px]:w-10/12  border-b flex items-center px-4 sm:px-8 `}
    >
      {!isMaxWidth900 && (
        isOpen ? <p className="bg-red-100 py-1 px-5  rounded-md"  onClick={() => setIsOpen(false)} >Close</p> :
        <Icon
          as={HamburgerIcon}
...
{!isOpen && (
  <>
        ...
        {toCapital.includes(title.toUpperCase()) ? title.toUpperCase() :  title.charAt(0).toUpperCase() + title.slice(1).toLowerCase()}
      </h3>
